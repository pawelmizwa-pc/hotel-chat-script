...
export class ChatHandler {
  ...
  async processChat(chatRequest: ChatRequest): Promise<ChatResponse> {
    ...
    const [firstAndSecondResponse, thirdResponse] = await Promise.allSettled([
      guestServiceTask
        .execute({
          userMessage,
          sessionHistory: collectedData.sessionHistory,
        ...
        .then(async (result) => {
          console.log("Guest service task result:", result);
          const buttonsResult = await buttonsTask.execute({
            userMessage,
            excelData: excelDataResult.excelData,
            ...
            firstCallOutput: result.content,
            trace,
          });
          return {
            firstTaskResult: result,
            buttonsTaskResult: buttonsResult,
          };
        }),
      emailTask.execute({
        userMessage,
    ...
    if (firstAndSecondResponse.status === "fulfilled") {
      buttons = firstAndSecondResponse.value.buttonsTaskResult.buttons;
      detectedLanguage =
        firstAndSecondResponse.value.buttonsTaskResult.language;
      buttonsUsage = firstAndSecondResponse.value.buttonsTaskResult.usage;
    } else if (firstAndSecondResponse.status === "rejected") {
      console.error(
        "Guest service task failed:",
        firstAndSecondResponse.reason
      );
      throw new Error(
        `Guest service task failed: ${
          firstAndSecondResponse.reason instanceof Error
            ? firstAndSecondResponse.reason.message
            : String(firstAndSecondResponse.reason)
        }`
      );
    }
    const guestServiceResult = firstAndSecondResponse.value.firstTaskResult;
